;(function(ng){
	...
	ng.module('angular.autosuggest',[])
	
	/* 
		CREDIT TO BENNADEL FOR HTTPI FACTORY
		@author - Bennadel
		@uri https://github.com/bennadel/httpi
	*/
	.factory(
		"httpi",
	...
	.directive('autoSuggestItem',function(){
		return{
			require: '^autoSuggest',
			link: function($scope,$element,$attr,controller){
				// Setting tabindex to 0 all
				// items to make sure they are 
				// listening to keyboard events
				$element.attr('tabindex',0);

				// Updating parent controller with index and item
				controller.updateTotalItems($scope.$index,$scope.$eval($attr.autoSuggestItem));

				// Bind click event on each item and 
				// call callback on click
				$element.bind("click",function(){
					controller.activateIndex($scope.$index);
				});

			}
		}
	})
	.directive('autoSuggest',['httpi',function(httpi){
		return{
			restrict: 'E',
			replace: true,
			transclude: true,
			template: '<div class="angular-autosuggest"><div ng-transclude></div></div>',
			...
			link: function($scope, $element, $attr){
				...
				// Hash containing autosuggest items and their indexes
				$scope.autoSuggestItemsHash = {};

				// Auto suggest current active index
				$scope.activeIndex = 0;

				// Hash to store search query
				$scope.search = {};

				// Current status of http request
				$scope.search.status = 'idle';

				// pendingRequest object
				var pendingRequest = null;

				// the callback method to run after 
				...
				$scope.callback = function(){
					...
					if(typeof(fun) == 'function'){
						...
						// Warn if no callback function
						console.warn('autoSuggest : ','searchSelectCallback is not a valid function')
					}
				};

				// Updating current index
				$scope.updateActiveIndex = function(index){
					// evaluating $socpe to refer active index
					// changes on UI.
					...
					if(e.which == 40 || e.which == 38){
						...
						if(e.which == 38){
							...
							if($scope.activeIndex > 0){
								// If currentIndex is greater then zero
								// Then reduce it by one
								$scope.updateActiveIndex($scope.activeIndex-1);
							}
							else{
								// Else set it to largest item
								$scope.updateActiveIndex($scope.autoSuggestItemsCount-1);
							}
						}
						else{
							if($scope.activeIndex+1 < $scope.autoSuggestItemsCount){

								// If currentIndex+1 is less then total length
								// Then reduce it by one
								$scope.updateActiveIndex($scope.activeIndex+1);
							}
							else{
								// Else set it back to zero
								$scope.updateActiveIndex(0);
							}
						}
						return false;
					}
